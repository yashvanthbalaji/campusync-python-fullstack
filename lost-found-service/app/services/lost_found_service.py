...
def report_item(reporter_email, form_data, image_path, db):
    """
    ...
    run_matching_algorithm(item, db)
...
def run_matching_algorithm(new_item, db):
    """
    ...
    candidates = db.query(LostFoundItem).filter(
...
def confirm_resolved(item_id, db):
    """
    ...
    """
    item = db.query(LostFoundItem).get(item_id)
    if not item:
    ...
    if item.matched_with_id:
        partner = db.query(LostFoundItem).get(item.matched_with_id)
        if partner:
...
def get_my_items(email, db):
    """Returns items reported by the given email, newest first."""
    return db.query(LostFoundItem).filter_by(
        reporter_email=email
    ).order_by(LostFoundItem.created_at.desc()).all()


def get_potential_matches(db):
    """Returns all items currently flagged as POTENTIAL_MATCH."""
    return db.query(LostFoundItem).filter_by(match_status='POTENTIAL_MATCH').all()


def get_matched_pair_details(item_id, db):
    """
...
def update_status(item_id, new_status, db):
...
def update_item_status(item_id, new_item_status, db):
    """
...
def get_items_by_item_status(status, db):
    """Returns items filtered by item_status value."""
    return db.query(LostFoundItem).filter_by(item_status=status).all()
